...
export function buildOpenApiSpec(baseUrl: string) {
  ...
  const pokemonMoveLearnItemSchema = {
    ...
    properties: {
      ...
      method: { type: 'string', example: 'Nivel' },
      methodKey: { type: 'string', example: 'level-up' },
      versionGroup: { type: 'string', example: 'Scarlet Violet' },
      versionGroupKey: { type: 'string', example: 'scarlet-violet' },
      level: { type: 'integer', example: 7 },
    },
    required: ['move', 'moveSlug', 'type', 'typeKey', 'method', 'methodKey', 'versionGroup', 'versionGroupKey', 'level'],
  }
